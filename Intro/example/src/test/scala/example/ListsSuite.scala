...
class ListsSuite extends FunSuite {
   ...
   */
  test("details why one plus two is three") {
  ...
  test("sum of negative numbers gets a negative result") {
    assert(sum(List(-1, -5, -8)) === -14)
  }
  
  test("max of a few numbers picks the highest number") {
    assert(max(List(3, 7, 2)) === 7)
  }
  
  test("max of no numbers raises NoSuchElementException") {
    intercept[NoSuchElementException] {
      max(List())
    }
  }
  
  test("max of same number is the number") {
    assert(max(List(3,3,3)) === 3)
  }
  
  test("max of negative numbers is the least negative number") {
    assert(max(List(-1, -4, -5)) === -1)
  }
}
