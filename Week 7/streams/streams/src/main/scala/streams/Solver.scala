...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] = 
    b.legalNeighbors.map(blockMove => (blockMove._1, blockMove._2 +: history)).toStream
  ...
  lazy val pathsToGoal: Stream[(Block, List[Move])] =
    from(Stream((Block(goal, goal), List())), Set(Block(goal, goal)))
  ...
  lazy val solution: List[Move] = {
    val solutionPaths = pathsFromStart.filter(x => done(x._1))
    if (solutionPaths.isEmpty) List()
    else solutionPaths.head._2
  }
}
